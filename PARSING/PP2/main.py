...
def get_data(url):
    ...
    for item in range(1, 24):
        req = requests.get(url + f"&PAGEN_1={item}&PAGEN_2={item}", headers) # Pagination web site
        ...
        for project_url in projects_urls:
            req = requests.get(project_url, headers) #getting link for the percise page
            project_name = project_url.split("/")[-2] #make a name
